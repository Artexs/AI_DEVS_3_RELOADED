...
import { prompt as useSearchPrompt } from '../prompts/tools/web/useSearch';
import { prompt as askDomainsPrompt } from '../prompts/tools/web/askDomains';
import { prompt as selectResourcesToLoadPrompt } from '../prompts/tools/web/pickResources';
...
export class WebSearchTool {
  ...
  async isWebSearchNeeded(messages: ChatCompletionMessageParam[]) {
    ...
    const response = await this.langfuseService.llmRequestAsJson(
      'isWebSearchNeeded',
      [systemPrompt, ...messages],
      '4o'
    );
    return response as SearchNecessityResponse;
  }
    ...
    if (queries.length > 0) {
      ...
      docs = await Promise.all(searchResults.flatMap(searchResult => 
        searchResult.results.map(async (result) => {
          ...
          const doc = document(content, 'gpt-4o', {
            name: `${result.title}`,
